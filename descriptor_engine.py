...
def calculate_single_molecule_descriptors(
    molecule_id,
    smiles
):
    if pd.isna(smiles):
    ...
    result = {
        ...
        "Exact Molecular Weight": round(
            Descriptors.ExactMolWt(mol), 4
        ),

        "LogP": round(
            Descriptors.MolLogP(mol), 4
        ),
        ...
        "Fraction Csp3": round(
            Descriptors.FractionCSP3(mol), 4
        ),
        ...
        "Bertz CT": round(
            Descriptors.BertzCT(mol), 4
        ),

        "Balaban J": round(
            Descriptors.BalabanJ(mol), 4
        ),

        **atom_counts,

        "Total Chalcogen Count": (
            atom_counts["Oxygen Count"]
        ...
        "Contains S":
            int(atom_counts["Sulfur Count"] > 0),

        "Contains Se":
            int(atom_counts["Selenium Count"] > 0),

        "Contains Te":
            int(atom_counts["Tellurium Count"] > 0),
...
def create_fingerprint_dataset(input_df):
    ...
    for _, row in input_df.iterrows():
        ...
        result.update(
            calculate_fingerprint_columns(mol)
        )

        fingerprint_results.append(result)

    return (
        pd.DataFrame(fingerprint_results),
        pd.DataFrame(invalid_results)
    )
...
def run_molecular_descriptor_platform(
    ...
    valid_df, invalid_df, summary = (
        process_molecular_dataset(clean_input_df)
    )

    fingerprint_df, _ = (
        create_fingerprint_dataset(clean_input_df)
    )

    summary_df = pd.DataFrame([summary])

    descriptor_file = (
        f"{project_name}_descriptors.csv"
    )

    fingerprint_file = (
        f"{project_name}_fingerprints.csv"
    )

    invalid_file = (
        f"{project_name}_invalid_smiles.csv"
    )

    summary_file = (
        f"{project_name}_summary.csv"
    )

    zip_file = (
        f"{project_name}_complete_outputs.zip"
    )
